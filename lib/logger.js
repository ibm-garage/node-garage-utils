...
const appConfiguration = {
  appenders: {
    appOut: { type: 'stdout', layout: localLayout }
  },
  categories: {
    default: {
      appenders: ['appOut'],
...
function defaultConfiguration() {
  ...
  return process.env.VCAP_APPLICATION ? cfAppConfiguration : appConfiguration;
}

function configure(options = {}) {
  if (!configuration) {
    const config = options.type && configurations[options.type];
    configuration = config || defaultConfiguration();
    command = options.command || path.basename(appEnv.mainFile, '.js');
