...
describe("ResponseError()", () => {
  test("creates a response error with specified message and status", () => {
    const err = new errors.ResponseError(499, "Oopsie");
    expect(err).toMatchObject({ status: 499, message: "Oopsie" });
    expect(err.detail).toBeUndefined();
    expect(err.code).toBeUndefined();
    expect(err.cause).toBeUndefined();
  });

  test("creates a minimal response error by default", () => {
    const err = new errors.ResponseError();
    expect(err).toMatchObject({ status: undefined, message: "" });
    expect(err.detail).toBeUndefined();
    expect(err.code).toBeUndefined();
    expect(err.cause).toBeUndefined();
  });

  test("creates a response error with the specified status, message, detail, code, and cause", () => {
    const err = new errors.ResponseError(400, "Bad request", {
      detail: "Invalid body",
      code: "BR1000",
      cause,
    });
...
describe("responseError()", () => {
  test("returns a response error with specified message and status", () => {
  ...
  test("returns a minimal response error by default", () => {
  ...
  test("returns a response error with the specified status, message, detail, code, and cause", () => {
...
describe("toResponseError()", () => {
  ...
  describe("options:", () => {
    test("uses logger to log a non-response error at level error by default", () => {
      ...
      const err = errors.toResponseError(cause, { logger });
      expect(logger.error).toHaveBeenCalledWith(err);
    });

    test("includes logMessage in the log for a non-response error", () => {
      const logger = { error: jest.fn().mockName("error") };
      const err = errors.toResponseError(cause, { logger, logMessage: "Unhandled error" });
    ...
    test("passes multiple logMessage values to the logger to allow substitution", () => {
      ...
      const err = errors.toResponseError(cause, {
        logger,
        logMessage: ["Error %s in %s", 100, "module"],
      });
      expect(logger.error).toHaveBeenCalledWith(err, "Error %s in %s", 100, "module");
    });

    test("logs at logLevel by calling the logger function with that name", () => {
      const logger = { warn: jest.fn().mockName("warn") };
      const err = errors.toResponseError(cause, { logger, logLevel: "warn" });
    ...
    test("successfully logs with the real Bunyan logger", () => {
      ...
      const err = errors.toResponseError(cause, {
        logger,
        logLevel: "debug",
        logMessage: "Mock error",
      });
      expect(spy).toHaveBeenCalledWith(err, "Mock error");
      spy.mockRestore();
    });
  });
});
...
describe("toResponseBody()", () => {
  ...
  describe("options:", () => {
    test("includes a full stack trace if stack is true", () => {
      ...
      expect(body.stack).toMatch(/^ResponseError: Internal server error/);
    ...
    test("supports logging non-response errors with the same options as toResponseError()", () => {
      const logMessage = ["Error %s in %s", 100, "module"];
      const logger = { warn: jest.fn().mockName("warn") };
      const options = { logger, logLevel: "warn", logMessage };
      errors.toResponseBody(cause, options);
      expect(logger.warn).toHaveBeenCalledWith(expect.any(errors.ResponseError), ...logMessage);
    });
  });
});
