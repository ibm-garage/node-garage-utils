...
logging.formatter = options => {
  return formatTimestamp(options.timestamp) + formatLevel(options.level) +
    formatLabel(options.label) + formatMessage(options.message, options.meta);
};
...
logging.configureWinston = (cfAppEnv) => {
  const level = app.env.isDev ? 'debug' : 'verbose';
...
logging.configureWinstonForTests = (suppressConsoleWarn, filename = 'test.log') => {
  ...
  const transports = [];
  transports.push(new (winston.transports.File)({ level: 'silly', filename, json: false, formatter, timestamp }));
