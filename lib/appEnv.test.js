...
describe("_rootDir()", () => {
  ...
  describe("in a simulated application with garage-utils package installed in an external location", () => {
    ...
    test("finds the app root dir if the entry point is in a package in node-modules", () => {
      appEnv.mainFile = path.join(rootDir, "node_modules", "mocha", "bin", "mocha");
      expect(appEnv._rootDir()).toBe(rootDir);
    });
...
describe("_env()", () => {
  ...
  describe("when NODE_ENV is not set", () => {
    ...
    test("returns 'test' if the last segment of the main module is mocha, the current Mocha script name", () => {
      appEnv.mainFile = path.join(appEnv.rootDir, "node_modules", "mocha", "bin", "mocha");
      expect(appEnv._env()).toBe("test");
    });

    test("returns 'test' if the last segment of the main module is _mocha, the legacy Mocha script name", () => {
      appEnv.mainFile = path.join(appEnv.rootDir, "node_modules", "mocha", "bin", "_mocha");
      expect(appEnv._env()).toBe("test");
    });

    test("returns undefined if the last segment of the main module ends with, but is not, mocha", () => {
      appEnv.mainFile = path.join(appEnv.rootDir, "node_modules", "mocha", "bin", "notmocha");
      expect(appEnv._env()).toBeUndefined();
    });
