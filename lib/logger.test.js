...
describe("the logger created internally by _createLogger()", () => {
  ...
  describe("when the app env is tweaked to simulate an application in development", () => {
    let logger_;
    ...
    it("has a single raw stream named appout at debug level", () => {
      expect(logger_.streams).toHaveLength(1);
      const stream = logger_.streams[0];
  ...
  describe("when the app env is tweaked to simulate an application in production", () => {
    let logger_;
  ...
  describe("when the app env is tweaked to simulate an application in another non-dev environment", () => {
    let logger_;
  ...
  describe("when a non-default log format is specified", () => {
    let origLogFormat;
    ...
    afterAll(() => {
      if (typeof origLogFormat !== "undefined") {
    ...
    it("uses a valid value of the LOG_FORMAT environment variable", () => {
      process.env.LOG_FORMAT = "pretty";
      appEnv.rootDir = path.join("home", "app");
      appEnv.mainFile = path.join(appEnv.rootDir, "server", "server.js");
      appEnv.env = undefined;
      const logger_ = logger._createLogger();
      expect(logger_.streams[0]).toHaveProperty("type", "raw");
    });

    it("ignores an invalid LOG_FORMAT value and uses the default", () => {
      process.env.LOG_FORMAT = "invalid";
      appEnv.rootDir = path.join("home", "app");
      appEnv.mainFile = path.join(appEnv.rootDir, "server", "server.js");
      appEnv.env = undefined;
      const logger_ = logger._createLogger();
      expect(logger_.streams[0]).toHaveProperty("type", "stream");
    });
  });
});
