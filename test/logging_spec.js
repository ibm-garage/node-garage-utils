...
describe('logging', () => {
  describe('formatter()', () => {
    const ts =
      '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\\.[0-9]{2}Z|(?:[+\\-][0-9]{4})';
    ...
    it('formats a log entry with default timestamp, level, and message', () => {
      ...
      expect(logging.formatter(options)).to.match(
        new RegExp(ts + ' \\[INFO\\] Something happened')
      );
    });
    ...
    it('formats a log entry with default timestamp, level, label, and message', () => {
      ...
      expect(logging.formatter(options)).to.match(
        new RegExp(ts + ' \\[VERBOSE\\] \\[api\\] Some detail')
      );
    });
    ...
    it('formats a log entry with default timestamp, level, and error', () => {
      ...
      expect(logging.formatter(options)).to.match(
        new RegExp(ts + ' \\[ERROR\\] Error: Oops\n      at Context')
      );
    });
    ...
    it('formats a complete log entry with default timestamp, level, label, message, error, and metadata', () => {
      ...
      const expected =
        ts +
        " \\[ERROR\\] \\[api\\] Internal error { id: 1443, code: 'oops' }\n  Error: Oops\n      at Context";
